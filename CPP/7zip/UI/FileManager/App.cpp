...
void CApp::OnCopy(bool move, bool copyToSame, unsigned srcPanelIndex)
{
...

  {
    ...
    if (!destIsFsPath)
      useDestPanel = true;
    finalDestIsFsPath = destIsFsPath;
  ...
  const UString finalDestPath = destPath;
  bool openDestPathIsDir = true;
  if (indices.Size() == 1 && !destPath.IsEmpty() && !IS_PATH_SEPAR(destPath.Back()))
    openDestPathIsDir = false;
  ...
  if (openDestFolder && finalDestIsFsPath && result == S_OK)
  {
    UString openPath = finalDestPath;
    if (!openDestPathIsDir)
    {
      const int pos = openPath.ReverseFind_PathSepar();
      if (pos >= 0)
        openPath.DeleteFrom(pos + 1);
    }
    if (!openPath.IsEmpty())
      OpenFolderInExplorer(us2fs(openPath));
  }
}
